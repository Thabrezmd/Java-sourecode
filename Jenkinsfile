node {
  ...
  stage ('Deliver & Deployment') {
      sh 'curl -u admin:redhat@123 -T target/**.war "http://3.111.30.92:8080/manager/text/deploy?path=/uday&update=true"'
  }
  stage ('SmokeTest') {
      sh 'curl --retry-delay 10 --retry 5 "http://3.111.30.92:8080/uday"'
  }
}
